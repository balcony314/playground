...
| Embedding 服务 | BGE 风格 | 向量化（维度运行时探测）；本地演示为 compose 内 Ollama bge-m3 + [cmd/embedadapter](../cmd/embedadapter) 协议适配层 | `POST /embedding/{model}` |
| Rerank 服务 | SiliconFlow 风格 | 召回精排（top1）；本地演示为 compose 内 llama.cpp bge-reranker-v2-m3（同上适配层转发，异常降级恒等） | `POST /reranker/{model}` |
