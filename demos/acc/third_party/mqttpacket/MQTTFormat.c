...
char* MQTTFormat_toClientString(char* strbuf, int strbuflen, unsigned char* buf, int buflen)
{
	...
	case CONNACK:
	{
		...
		if (MQTTDeserialize_connack(&sessionPresent, &connack_rc, buf, buflen, &walk_len) == 1)
	...
	case PUBLISH:
	{
		...
		if (MQTTDeserialize_publish(&dup, &qos, &retained, &packetid, &topicName,
				&payload, &payloadlen, buf, buflen, &need_len, &walk_len) == 1)
	...
	case PUBCOMP:
	{
		...
		unsigned short packetid;
		if (MQTTDeserialize_ack(&packettype, &dup, &packetid, buf, buflen, &need_len, &walk_len) == 1)
	...
	case SUBACK:
	{
		...
		int grantedQoSs[1];
		if (MQTTDeserialize_suback(&packetid, maxcount, &count, grantedQoSs, buf, buflen, &walk_len) == 1)
	...
	{
		unsigned short packetid;
		if (MQTTDeserialize_unsuback(&packetid, buf, buflen, &walk_len) == 1)
...
char* MQTTFormat_toServerString(char* strbuf, int strbuflen, unsigned char* buf, int buflen)
{
	...
	case CONNECT:
	{
		...
		int rc;
		if ((rc = MQTTDeserialize_connect(&data, buf, buflen, &need_len, &walk_len)) == 1)
	...
	case SUBSCRIBE:
	{
		...
		if (MQTTDeserialize_subscribe(&dup, &packetid, maxcount, &count,
				topicFilters, requestedQoSs, buf, buflen, &need_len, &walk_len) == 1)
	...
	case UNSUBSCRIBE:
	{
		...
		int maxcount = 1, count = 0;
		MQTTString topicFilters[1];
		if (MQTTDeserialize_unsubscribe(&dup, &packetid, maxcount, &count, topicFilters, buf, buflen, &need_len, &walk_len) == 1)
